...
import dass  # register custom modules
from dass.models import (
    build_uda_segmentor,
)  # if uda in config, build uda segmentor, else mmseg segmentor
from dass.datasets import (
    build_uda_dataset,
)  # if st in config, build uda dataset, else mmseg dataset
from dass.apis import (
    train_uda_segmentor,
)  # modify build_dataset to build_uda_dataset for validate
...
def main():
    ...
    datasets = [build_uda_dataset(cfg.data.train)]
    if len(cfg.workflow) == 2:
        val_dataset = copy.deepcopy(cfg.data.val)
        val_dataset.pipeline = cfg.data.train.pipeline
        datasets.append(build_uda_dataset(val_dataset))
