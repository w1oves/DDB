...
class CKD(UDADecorator):
    ...
    def _init_sub_model_weights(self):
        ...
        if self.stu_model_load_from is not None:
            ...
            print_log(
                f"Load checkpoint for stu_model from {self.stu_model_load_from}",
                "mmseg",
            )
    ...
    def forward_train(
        ...
        if not self.soft_distill:
            ...
            stu_tgt_softmax = torch.log_softmax(stu_tgt_logits, dim=1)
            with torch.no_grad():
                cu_tgt_logits, cu_tgt_feature = self.encode_decode(
                    target_img, target_img_metas, "cu", return_feature=True
                )
            ...
            # if stu_tgt_loss.isnan().any():
            #     print(stu_tgt_loss,stu_tgt_softmax,ensemble_tgt_softmax)
            # if stu_src_loss.isnan().any():
            #     print(stu_tgt_loss,stu_tgt_softmax,ensemble_tgt_softmax)
            # if any(param.isnan().any() for param in self.get_model('stu').parameters()):
            #     print(stu_tgt_loss,stu_tgt_softmax,ensemble_tgt_softmax)
